...
class HomeController extends Controller
{
    ...
    public function checkScore(Request $request)
    {
      if($request->isMethod('post')){
        ...
        $nilai_kategori = array_fill(1,32,0);
        foreach($result['data'] as $nilai){
          $nilai_kategori[$nilai->idKategori]++;
        }
        $result['nilai_kategori'] = $nilai_kategori;

        $result['kategori_individu'] = Kategori::where('idKategori','<',19)->get();
        $result['kategori_kelompok'] = Kategori::whereBetween('idKategori',[19,29])->get();
        $result['kategori_angkatan'] = Kategori::where('idKategori','>', 29)->get();
    ...
    public function calculateFinalScore($nim)
    {
      ...
      $nilai_akhir = (($nilai_wajib) / $max_wajib * $presentase_wajib) + (($nilai_opsional+100) / $max_opsional * $presentase_opsional);
